...
interface ResourceFilePanelProps {
  ...
  /** 打开文件事件 */
  onOpenFile?: (fileResource: FileResourceTreeNodeRes) => void;
  /** 当前组件点击最小化事件 */
  onHidePanel?: () => void;
  /** 新增资源文件成功 */
  onAddResourceFile?: (file: FileResourceTreeNodeRes) => void;
  /** 删除资源文件成功 */
  onDelResourceFile?: (files: Array<FileResourceTreeNodeRes>) => void;
}
...
class ResourceFilePanel extends React.Component<ResourceFilePanelProps, ResourceFilePanelState> {
  ...
  public reLoadTreeData(spin: boolean = true, triggerOnSelectChange: boolean = true) {
    ...
    request.get(FastApi.ResourceFileManage.getFileTree, { params: { module: 1 } })
      .then(treeData => {
  ...
  /** 新增文件 */
  private addFile() {
    const { onAddResourceFile } = this.props;
    const { expandedIds, addResourceFileForm } = this.state;
    this.setState({ addResourceFileLoading: true });
    request.post(FastApi.FileResourceManage.addFile, { ...addResourceFileForm, module: 1 })
      .then((res: Array<FileResource>) => {
        res?.forEach(item => {
          ...
          if (onAddResourceFile) onAddResourceFile(item);
          this.setState({ selectedId: item.id });
        });
        this.setState({ showAddResourceFileDialog: false });
        this.reLoadTreeData(false);
      }).finally(() => this.setState({ addResourceFileLoading: false }));
  }
  ...
  private addDir() {
    ...
    request.post(FastApi.FileResourceManage.addDir, { module: 1, fullPath: path })
  ...
  /** 删除HttpApi */
  private delFile() {
    const { onDelResourceFile } = this.props;
    ...
    request.delete(FastApi.FileResourceManage.delFile, { params: { id: nodeData.id } })
      .then((res: Array<FileResource>) => {
        ...
        if (onDelResourceFile) onDelResourceFile(files);
        this.setState({ showDeleteDialog: false });
        this.reLoadTreeData(false);
    ...
    return (
      <Menu className={cls(styles.menu)}>
        <MenuItem
          ...
          text="新增文件"
          onClick={() => {
            const addResourceFileForm: AddResourceFileForm = { path: "/", name: "" };
            const nodeData = contextMenuSelectNode?.nodeData;
            if (nodeData) addResourceFileForm.path = nodeData.isFile === 1 ? nodeData.path : (nodeData.path + nodeData.name);
            if (!addResourceFileForm.path.endsWith("/")) addResourceFileForm.path += "/";
            this.setState({ showAddResourceFileDialog: true, addResourceFileForm: addResourceFileForm });
          }}
        />
        <MenuItem
  ...
  private getAddFileDialog() {
    const { showAddResourceFileDialog, addResourceFileForm: { path, name }, addResourceFileLoading } = this.state;
    return (
      <Dialog
        ...
        usePortal={true}
        isCloseButtonShown={!addResourceFileLoading}
        canEscapeKeyClose={!addResourceFileLoading}
        canOutsideClickClose={false}
        autoFocus={true}
        enforceFocus={true}
        isOpen={showAddResourceFileDialog}
        onClose={() => this.setState({ showAddResourceFileDialog: false })}
      >
        <FormGroup style={{ marginTop: 12 }} inline={true} label={"所属目录"}>
          <InputGroup
            type={"text"}
            placeholder={"输入所属目录"}
            disabled={addResourceFileLoading}
            value={path}
            onChange={e => this.setState({ addResourceFileForm: { path: e.target.value, name } })}
          />
        </FormGroup>
        <FormGroup style={{ marginBottom: 12 }} inline={true} label={"文件名称"}>
          <InputGroup
            ...
            autoFocus={true}
            value={name}
            onChange={e => this.setState({ addResourceFileForm: { path, name: e.target.value } })}
          />
        </FormGroup>
        <div className={Classes.DIALOG_FOOTER}>
          <div className={Classes.DIALOG_FOOTER_ACTIONS}>
            <Button onClick={() => this.setState({ showAddResourceFileDialog: false })} disabled={addResourceFileLoading}>取消</Button>
            <Button intent={Intent.PRIMARY} onClick={() => this.addFile()} loading={addResourceFileLoading}>确认</Button>
          </div>
        </div>
      </Dialog>
    );
