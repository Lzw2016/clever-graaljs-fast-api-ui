...
import styles from "./InitScriptPanel.module.less";
...
interface InitScriptPanelState {
  /** 数据加载状态 */
  loading: boolean;
  /** 树数据 */
...
// 组件状态默认值
const defaultState: InitScriptPanelState = {
  loading: true,
  treeData: [],
  expandedIds: new Set(),
  selectedId: "",
...
class InitScriptPanel extends React.Component<InitScriptPanelProps, InitScriptPanelState> {
  /** 执行组件状态的全局锁 */
  private saveStateLock: boolean = false;
  /** 保存组件的状态 */
  private saveComponentState = lodash.debounce(() => this.saveState().finally(), 1_000, { maxWait: 3_000 });

  constructor(props: InitScriptPanelProps) {
    super(props);
    this.state = { ...defaultState, ...storeGetData(componentStateKey.InitScriptPanelState) };
  }
  ...
  public async saveState(): Promise<void> {
    ...
    await storeSaveData(
      componentStateKey.InitScriptPanelState,
  ...
  public reLoadTreeData(spin: boolean = true, triggerOnSelectChange: boolean = true) {
    ...
    request.get(FastApi.ResourceFileManage.getFileTree, { params: { module: 2 } })
      .then(treeData => {
  ...
  private addFile() {
    ...
    request.post(FastApi.FileResourceManage.addFile, { ...addFileForm, module: 2 })
  ...
  private addDir() {
    ...
    request.post(FastApi.FileResourceManage.addDir, { module: 2, fullPath: path })
...
export type { InitScriptPanelProps, InitScriptPanelState };
export { InitScriptPanel };
